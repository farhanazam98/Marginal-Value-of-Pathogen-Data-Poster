...
\begin{columns}
  ...
  \block{1. Background \& Motivation}{%
    ...
    \begin{itemize}
      ...
      \item These models can be used to create pathogens that are capable of more harm than their natural counterparts.
      \item With the growth of biological datasets, pathogen data access controls have been proposed to limit the proliferation of these capabilities.
      \item To justify the cost of pathogen data access controls, we seek to quantify the expected increase in dual-use capabilities from future data
    \end{itemize}
    \vspace{0.5em}
  ...
  \block{2. Research Question}{%
    ...
    \vspace{0.6em}
    \begin{itemize}
      \item \textbf{Data:} Protein sequence data from UniRef100 yearly archives, 2010--2026.
      \item \textbf{Model:} Position-Specific Scoring Matrix (PSSM), an alignment-based model that scores biological sequences using multiple sequence alignments (MSA)
...
\begin{columns}
  ...
  \block{4. Preliminary Results}{%
    ...
    \begin{tikzfigure}[{Additional protein sequence data does not seem to reliably increase mutation-effect prediction accuracy on SARS-CoV-2 proteins. For fitness of SARS-CoV-2 main protease, performance increases marginally. For binding and expression of SARS-CoV-2 spike protein, additional data hinders performance.}]
  ...
  \block{5. Limitations}{%
    ...
    \begin{itemize}
      \item For the preliminary results, we did not implement the alignment selection heuristic outlined in Gurev et al. (2025). Instead, we hardcoded a bitscore-per-residue value of 0.1.
      \item These results are specific to SARS-CoV-2. We would like to run this pipeline on additional viruses to see whether the trend generalizes.
    \end{itemize}
  }
  ...
  \block{6. Discussion}{%
    ...
    \begin{itemize}
      \item Our hypothesis for why additional data doesn't improve performance has to do with the quality of the data. Redundant protein sequence data might be adding noise.
